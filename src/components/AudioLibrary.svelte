...
<section class="mb4">
    <h2 class="h2 m0 mb1">Audio Library</h2>
    {#if !isAddingMotion}
    ...
    <div class="flex items-baseline">
        <label for="audio-search" class="m0 mt2">Filter:
            <input id="audio-search" type="text" bind:value={searchQuery} on:input={search} placeholder="Type in your query here">
        </label>
        <div>
            ...
            <section class="audio-group">
                {#each audioByGroups[groupName] as audio}
                    <div>
                        ...
                        {#if inEditingMode}
                            <button class="m0 mb1" on:click|preventDefault={removeAudio(audio.ID)}>Remove</button>
                        {/if}
                    </div>
                {/each}
            </section>
